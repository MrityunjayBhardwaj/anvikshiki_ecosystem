╔══════════════════════════════════════════════════════════╗
║  ĀNVĪKṢIKĪ v3.27 — ENFORCED STAGE GATE PROTOCOL         ║
╠══════════════════════════════════════════════════════════╣
║                                                          ║
║  This meta-prompt produces a guide in 8 STAGES.          ║
...
chapter sequence with structures and action titles, emotional 
beat sequence, confusion-pair registry, landscape point 
registry, decay marker registry, Forward Reference Matrix 
connections for this session, and resolution ledger status. 
Also reproduce the Stage 3 Reference Bank (chapter-by-source 
mapping), the Voice Calibration Document, all chapter 
fingerprints from prior sessions, and the current Forward 
Hook Ledger. If these are not in your context, request the 
user to paste them.
...
# The Ānvīkṣikī Meta-Prompt for Instructional Design — v3.27
...
**v3.26 change — Three-layer derivation system:** The full 6-component derivation is too heavy for both architecture planning and reader-facing prose. v3.26 splits the derivation into three layers:
...
STAGE 2: Phase 2 (Knowledge Architecture)
   → Output: Vyāpti list + Hetvābhāsa list + Dependency map (with MECE) +
             Threshold concepts + Spaced return plan + Resolution ledger +
             Chapter sequence + Structure assignments + Action titles +
             Emotional beats + Debate plans + Forward Reference Matrix +
             Exit Narrative + Session Plan
   → Verify before proceeding

PRE-STAGE 3: Voice Calibration Document (Session 0)
   → Output: 500–800 word voice sample establishing guide's tone
   → Verify before proceeding
...
STAGE 4: Phase 3a (Guide Opening + Part I: Foundations)
   → Input: Full Stage 1 + Stage 2 outputs restated as header + 
            Stage 3 Reference Bank + Voice Sample + Session Start Protocol
   → Output: Opening + Foundational chapters (with references 
             drawn from the Reference Bank) + Chapter Fingerprints + 
             Forward Hook Ledger updates
...
STAGE 5: Phase 3b (Part II: Intermediate Synthesis + Part III: Advanced Integration)
   → Input: Stage 2 architecture + Stage 3 Reference Bank + 
            All chapter fingerprints + Voice Sample + 
            Forward Hook Ledger + recalibration note
   → Output: Synthesis + Advanced chapters + Fingerprints + 
             Ledger updates
   → Run inline quality assertions before proceeding

STAGE 6: Phase 3c (Part IV: Frontier + Part V: Framework)
   → Input: Stage 2 architecture + Stage 3 Reference Bank + 
            All chapter fingerprints + Forward Hook Ledger +
            resolution ledger status + 
            cumulative structural element checklist
   → Output: Frontier section (with scenario forks) + 
             Expert's Framework chapter + Appendix assembly
   → Run inline quality assertions

STAGE 7: Phase 4 (Quality Verification + Assembly)
   → Input: Full guide assembled from Stages 4–6 + all fingerprints
   → Output: Verified final guide with all gaps identified and filled

STAGE 8: Phase 5 (Safety Net Revision)
   → Input: All fingerprints + Forward Hook Ledger + completed guide
   → Output: Revision manifest (should be EMPTY if prevention 
             mechanisms worked) + diagnostic for any failures
```
...
```
╔══════════════════════════════════════════════════════════════════════════╗
║        THE ĀNVĪKṢIKĪ INSTRUCTIONAL DESIGN META-PROMPT v3.27             ║
...
EPISTEMIC STATUS (required):
  ...
  Confidence in EXISTENCE of regularity: 
    [high / moderate / low]
  
  Confidence in SPECIFIC FORMULATION: 
    [high / moderate / low]
  
  Evidence base quality: 
    [experimental / quasi-experimental / observational / 
    case-based / theoretical / expert consensus]
  
  Key uncertainty: 
    [What aspect of this vyāpti is the generator LEAST 
    confident about? Be specific — not "might not always 
    hold" but "the threshold numbers may vary ±30% by 
    industry context"]

GUIDE RENDERING RULE: 
When this vyāpti appears in the guide, language must 
reflect confidence level:
  - High confidence → state as structural regularity 
    ("organizations consistently face...")
  - Moderate confidence → calibrated hedge 
    ("evidence suggests...", "in most observed cases...")
  - Low confidence → working hypothesis 
    ("current thinking holds...", "preliminary evidence 
    indicates...")

This ensures the guide MODELS the calibration behavior 
it teaches. The guide demonstrates calibrated confidence, 
not false precision.
```
...
───────────────────────────────────────────────────────────
MECE ASSERTION (at each branching point in the dependency map)
───────────────────────────────────────────────────────────

For every set of sibling concepts at the same structural 
tier — concepts that branch from the same parent or 
occupy the same level — perform:

MECE CHECK at [branch point]:
  Parent / tier: [what these siblings decompose]
  Siblings: [A] | [B] | [C] | ...

  COLLECTIVELY EXHAUSTIVE? [Yes / No]
    If No — what's missing:
    "Domain area [X] is uncovered. Resolution:
      (a) Add chapter/section covering [X]
      (b) Explicitly exclude [X] with documented rationale
      (c) Distribute [X] across [A] and [B] with clear 
          assignment documented"

  MUTUALLY EXCLUSIVE? [Yes / No / Partial — acceptable]
    If No or Partial — where they overlap:
    "[A] and [B] both cover [area]. Resolution:
      (a) Assign to one sibling, scope other to exclude
      (b) Acknowledge deliberate overlap — [A] covers from 
          [perspective 1], [B] from [perspective 2]
      (c) Create discrimination case (confusion-pair)"

RULES:
- Exhaustiveness is MANDATORY — missing areas must be 
  resolved before proceeding
- Exclusivity is ADVISORY — overlaps must be deliberate 
  and documented, not eliminated artificially
- Typical count: 3–5 checks per guide
- Common missing siblings to watch for:
  Business guides: Pricing, Operations, Communication
  Technical guides: Error handling, Performance, Security
  Any guide: "How to continue learning" meta-skill
...
**NEW IN v3.0, EXPANDED IN v3.26** — For each chapter in the planned sequence, assign a structure, an action title, an emotional beat, and (for debate chapters) a debate plan using the decision table and specifications below. Do not leave these to generation-time judgment.

**ACTION TITLE (required for each chapter):**

A complete clause or sentence stating the chapter's core 
insight — not its topic. Must pass three tests:

TEST 1 — DISAGREEMENT: Could someone who understood the 
domain argue against this title? 
  ✗ "Understanding Financial Statements" — no one argues
  ✓ "Financial Statements Tell a Story — and the Story 
    May Be Misleading"

TEST 2 — DOMAIN SPECIFICITY: Could this title serve as 
heading in a different domain's guide?
  ✗ "Why Context Matters" — any guide
  ✓ "Prices Are Discovered by Competing Pressures, Not 
    Set by Sellers" — only economics/business

TEST 3 — COMPRESSION: Is this recognizable as natural-
language rendering of the chapter's core vyāpti?
  V1 → "A Business That Consumes More Than It Creates 
    Is Dying — Regardless of Revenue"

THRESHOLD CONCEPTS: Longer titles acceptable — capture 
the SHIFT, not just the destination concept.

ACTION TITLES IN THE GUIDE: These become the actual 
chapter headings. The table of contents uses action titles, 
giving the reader a preview of the intellectual arc.

CONNECTION TO PART V: If the guide has a Compression 
Exercise, action titles are the reference point. The 
exercise asks readers to reproduce them from memory.

**EMOTIONAL BEAT (required for each chapter):**

| Beat | Reader Experience | When to Use |
|------|-------------------|-------------|
| CHALLENGE | "This is hard, but I see why it matters" | Complex new frameworks, cognitive restructuring |
| REWARD | "Oh — that's why!" | Concepts connecting, insights clicking | 
| REST | "I can do this" | Consolidation, application to familiar territory |
| THRESHOLD | "My prior understanding was wrong" | ONLY for threshold concepts (TC1, TC2, TC3...) |
| EXPANSION | "I can think about things I couldn't before" | New capability unlocked, scope widens |

SEQUENCING CONSTRAINTS (mandatory — verify at Stage 2 gate):

CONSTRAINT 1 — NO EXTENDED DIFFICULTY:
  Maximum TWO consecutive CHALLENGE beats without a 
  REWARD or REST between them. If logical dependency 
  forces three: insert brief REST consolidation, 
  restructure a middle chapter to include REWARD, 
  or reorder if dependency allows.

CONSTRAINT 2 — REGULAR REWARD:
  At least ONE REWARD in every 4-chapter sliding window.

CONSTRAINT 3 — THRESHOLD RESOLUTION:
  Every THRESHOLD must be FOLLOWED within 2 chapters 
  by REWARD or EXPANSION.

CONSTRAINT 4 — PART CULMINATION:
  Each Part ends with EXPANSION or REWARD (not CHALLENGE 
  or THRESHOLD).

EXAMPLE ARC (12 chapters):
Ch1:CHALLENGE → Ch2:CHALLENGE → Ch3:REWARD → 
Ch4:REST → Ch5:EXPANSION → Ch6:CHALLENGE → 
Ch7:CHALLENGE → Ch8:THRESHOLD → Ch9:REWARD → 
Ch10:EXPANSION → Ch11:THRESHOLD → Ch12:REWARD

**DEBATE PLANNING (required for each chapter with a planned debate):**

CHAPTER [N] DEBATE:
  Topic: [what experts disagree about]
  
  CRUX STATEMENT: [Single sentence identifying the specific 
  empirical or philosophical point where positions diverge. 
  If you cannot write this sentence, this may be a false 
  debate.]
  
  Position A: [specific claim]
  Position B: [specific counterclaim]
  
  CRUX TYPE:
  □ Empirical — resolvable with data not yet available
  □ Values-based — different weightings of legitimate goals
  □ Scope-based — both true in different conditions
  □ Definitional — same term used differently
  
  RESOLUTION RULES:
  - If Scope-based → convert to CONTEXTUAL ANALYSIS. 
    Honestly identify when each position holds. Do NOT 
    present as a debate if context dissolves disagreement.
  - If Definitional → resolve definition first, then check 
    if debate survives. Often it doesn't.
  - Only Empirical and Values-based cruxes produce genuine 
    debates worthy of debate format in the guide.
...
**NEW IN v3.2, REVISED IN v3.26** — Identify claims in the planned guide that are non-obvious: they rest on multi-step inference, combine multiple vyāptis, or lack direct source grounding. These claims require explicit derivation proof sketches in the architecture and reader-facing derivation summaries in the guide text.
...
Produce:
1. The Vyāpti List (5–10 invariable relations, fully formatted with causal classification and epistemic status)
2. The Hetvābhāsa List (5–8 fallacies, fully formatted)
3. The Concept Dependency Map (DAG with circular dependency resolutions, ≈ confusion-pairs noted, and MECE assertions at branch points)
...
7. The Chapter Sequence with structure assignments, action titles, emotional beats, and debate plans (from Step 2G)
...
13. The Forward Reference Matrix (from Step 2N)
14. The Exit Narrative (from Step 2O)
15. The Session Plan (from Step 2P)

### Step 2N: Forward Reference Matrix

**NEW IN v3.26** — For each chapter pair (earlier → later) where a conceptual connection exists, plan the reference before generation:

```
FORWARD REFERENCE MATRIX:

FROM       → TO         TYPE              PLANNED RENDERING
Ch [N]    → Ch [M]     [type]            "[seed to plant in Ch N]"
Ch [N]    → Ch [M]     [type]            "[seed to plant in Ch N]"
...

CONNECTION TYPES:

FORESHADOW: "This concept has a deeper dimension — 
  Chapter [M] will explore it"
  Use when: later chapter deepens an idea introduced here

INCOMPLETE PICTURE: "This framework is deliberately 
  incomplete — Chapter [M] adds the missing piece"
  Use when: later chapter adds essential dimension the 
  reader needs to know is coming

CONCEPT EXTENSION: "This idea applies more broadly — 
  Chapter [M] extends it to [new domain]"
  Use when: same principle operates in a different context

SYNTHESIS: "This element combines with others in 
  Chapter [M] to produce [integrated capability]"
  Use when: later chapter integrates multiple prior concepts

TENSION: "This principle seems to conflict with Chapter [M] 
  — the resolution reveals something important"
  Use when: apparent contradiction between chapters creates 
  productive cognitive tension

USAGE DURING GENERATION:

1. When generating the EARLIER chapter: plant the seed. 
   Use the planned rendering from the matrix. The seed 
   references the later chapter's ARCHITECTURAL content 
   (vyāpti, action title, concept) — not its prose.

2. When generating the LATER chapter: harvest the seed. 
   Explicitly connect back to the earlier chapter's 
   planted reference using consistent terminology from 
   the fingerprint's first-mention registry.

3. Each planted seed is tracked in the Forward Hook 
   Ledger (C12) and the chapter fingerprint's Forward 
   Hooks section.
```

### Step 2O: Exit Narrative

**NEW IN v3.26** — A ~150–200 word prose paragraph simulating the COMPLETED READER in a specific realistic scenario. Not a competency list — a NARRATIVE showing competencies in action.

```
FORMAT:

"[Reader role/context, from Phase 1 calibration] encounters 
[specific messy, ambiguous real-world situation with 
incomplete information, time pressure, competing concerns].

Before this guide, they would have [specific inferior 
approach — name H-numbers they'd fall into, V-numbers 
they'd miss, tacit judgment they'd lack]. They would 
likely [specific pre-guide action].

After this guide, they [sequence of cognitive moves]:
First, they recognize [V-number pattern]. They check 
[scope condition]. They resist [H-number — name the tell]. 
They apply [framework from specific chapter]. They arrive 
at [calibrated conclusion with appropriate uncertainty].

The difference is not more facts or memorized frameworks. 
The difference is [single sentence capturing the guide's 
...
□ Every V-number referenced → assigned to a chapter? 
  [list each: V__ → Ch __ ✓/✗]
□ Every H-number referenced → assigned to a chapter?
  [list each: H__ → Ch __ ✓/✗]
□ Every tacit skill referenced → TK block planned?
  [list each: skill → Ch __ ✓/✗]
□ Scenario realistic for reader's operating context?
□ "Before" matches calibrated starting level?
□ All cognitive moves achievable with guide's content?
□ Final sentence captures EMERGENT capability (not 
  single-chapter)?

If ANY check fails → architectural gap. Resolve BEFORE 
proceeding past Stage 2 gate.

───────────────────────────────────────────────────────────
SECONDARY USES:
───────────────────────────────────────────────────────────

1. OPENING ("What You Will Be Able to Do"): Final sentence 
   becomes the guide's most concrete promise
2. PART V DIAGNOSTIC QUESTIONS: Each cognitive move implies 
   a diagnostic question
3. FINAL SIMULATION: Scenario (adapted) becomes the guide's 
   capstone transfer test with AAR
4. SCENARIO FORK CONNECTION: If scenario involves a Part IV 
   open question, the relevant fork provides the 
   decision framework under uncertainty

The exit narrative is an ARCHITECTURAL tool. It does NOT 
appear in the guide. It feeds elements that do.
```

### Step 2P: Session Plan

**NEW IN v3.26** — Assign chapters to generation sessions with these constraints:

```
1. MAX CHAPTERS PER SESSION:
   - THRESHOLD or high-density CHALLENGE chapters: max 2
   - REST, REWARD, EXPANSION chapters: max 3

2. FRONT-LOAD DIFFICULTY: Within each session, generate 
   harder chapters first (when creative energy is highest).

3. SEPARATE OPENING AND CHAPTER 1: The guide's opening 
   (Stage 3) and Chapter 1 both require peak creative 
   energy and voice calibration. Different sessions.

4. PART TRANSITIONS AT SESSION BOUNDARIES: Recalibration 
   checks function better at session start than mid-session.

5. FINGERPRINTING WITHIN SESSION: Never defer fingerprinting 
   to the next session — voice and detail memory is freshest 
   immediately after generation.

SESSION MAP TEMPLATE:

Session 0: Voice Calibration Document
Session 1: Opening (Stage 3)
Session 2: Chapter 1 [+ Chapter 2 if REST/REWARD]
Session 3: Chapters [N]–[M] (following constraints above)
...
Session K-1: Final chapter + Part V + Appendix assembly
Session K: Stage 7 verification ONLY (no generation)
```
...
□ Each vyāpti has epistemic status (C8)? [yes/no]
...
□ MECE checks at all major branching points (minimum 3)? [yes/no]
□ All exhaustiveness gaps resolved? [yes/no]
□ All overlaps documented as deliberate or assigned? [yes/no]
...
□ Every chapter has an action title passing all three tests 
  (Disagreement, Domain Specificity, Compression)? [yes/no]
□ Threshold concept chapters capture the shift? [yes/no]
□ Every chapter has emotional beat assigned? [yes/no]
□ No 3+ consecutive CHALLENGE beats? [yes/no]
□ REWARD in every 4-chapter sliding window? [yes/no]
□ Every THRESHOLD followed within 2 chapters by REWARD/EXPANSION? [yes/no]
□ Each Part ends on EXPANSION or REWARD? [yes/no]
□ All TC chapters assigned THRESHOLD beat (not CHALLENGE)? [yes/no]
□ All planned debates have crux statements? [yes/no]
□ Scope-based "debates" reclassified as contextual analyses? [yes/no]
□ Definitional "debates" resolved or reclassified? [yes/no]
...
□ Forward Reference Matrix (Step 2N): produced? 
  All cross-chapter connections planned with types? [yes/no]
□ Exit Narrative (Step 2O): produced (~150–200 words, concrete prose)?
  References specific V-numbers and H-numbers? [yes/no]
  All verification checks pass? [yes/no]
  All gaps resolved? [yes/no]
□ Session Plan (Step 2P): produced?
  Max chapters per session respected? [yes/no]
  Part transitions at session boundaries? [yes/no]

Fix ANY gaps found. Then STOP and present ALL 15 architecture 
...
# ═══════════════════════════════════════════
# PRE-STAGE 3: VOICE CALIBRATION DOCUMENT
# ═══════════════════════════════════════════

**NEW IN v3.26** — Before generating ANY guide content (including the Opening), generate a VOICE SAMPLE — a 500–800 word prose passage that establishes the guide's voice without being part of the guide.

**PURPOSE:** Solves the cold-start problem. The generator needs examples of the guide's voice to produce consistent tone from Chapter 1. Without a voice sample, Chapter 1 bears the cost of voice calibration and is weaker than later chapters. The voice sample gives Chapter 1 the same reference point that Chapter 10 gets from nine prior chapters.

**VOICE SAMPLE FORMAT:**

Generate ONE of the following (choose based on domain):

OPTION A — MIDDLE-CHAPTER EXCERPT:
  Write 500–800 words AS IF you were writing a section 
  from a chapter in the middle of the guide (Chapter 6–8 
  equivalent). Choose a concept from that chapter's vyāpti 
  assignment. Write it with full voice commitment — 
  analogies, reader address, simulation snippet, tacit 
  knowledge aside. This is a practice chapter that will 
  be DISCARDED and regenerated properly in sequence.

OPTION B — AUTHOR'S NOTE:
  Write a hypothetical author's note for the guide — the 
  voice the author would use in a preface. This establishes 
  the relationship between guide and reader: confidence 
  level, formality, warmth, intellectual ambition.

OPTION C — EXPERT CONVERSATION:
  Write a passage where the guide explains a concept from 
  Part II or III as if in conversation with the reader. 
  Include: one analogy from the reader's domain, one 
  moment of "here's what makes this non-obvious," one 
  tacit knowledge aside. This directly demonstrates the 
  guide's explanatory voice.

**VOICE SAMPLE REQUIREMENTS:**

The voice sample MUST demonstrate all of:

□ CONFIDENCE LEVEL: How much does the prose trust the 
  reader? (Should match Phase 1 calibration of reader's 
  starting level)

□ ANALOGY INTEGRATION: How are analogies from the reader's 
  domain woven in? (Forced and labeled? Or organic and 
  embedded?)

□ DENSITY CALIBRATION: How much is explained vs. trusted? 
  (For an advanced reader: less explanation, more insight. 
  For a beginner: more scaffolding.)

□ PROSE RHYTHM: Sentence length variation. When to use a 
  short sentence for impact. When to let a complex idea 
  breathe across a longer construction.

□ EMOTIONAL REGISTER: Authoritative but warm? Precise but 
  not dry? Challenging but not condescending?

□ READER ADDRESS: Direct second-person? Inclusive "we"? 
  Impersonal?

**USAGE:**

- The Voice Sample is included in context for ALL subsequent 
  generation sessions (Stage 3 onward)
- It is referenced by the Session Start Protocol at 
  the start of every session
- It is NOT included in the guide itself
- It IS included in context compaction as HIGH-PRIORITY 
  content (alongside chapter fingerprints)
- If the voice naturally evolves during generation (it will, 
  slightly), the Voice Sample still serves as the BASELINE — 
  the reference point that prevents drift from becoming 
  inconsistency

═══════════════════════════════════════════
⛔ VOICE CALIBRATION GATE — HARD STOP
═══════════════════════════════════════════

SELF-AUDIT:
□ Voice sample produced (500–800 words)?
□ All 6 requirements demonstrated?
□ Voice matches Phase 1 calibration of reader level?

STOP and present the Voice Sample. Tell the user:
"This voice sample establishes the guide's tone. It will 
not appear in the guide itself. Reply 'continue' to proceed 
to Stage 3 (Research Gate), or specify voice adjustments."
...
**Every entry must record how well it was located.** A bank of
title/author/year strings is well-formed and entirely unverifiable: nothing
downstream can confirm the work exists, and a citation that was looked up and
one that was invented are byte-identical once written. So each entry carries a
**machine-checkable identifier** — or an explicit statement that there is
nothing to locate.

**The four resolution statuses.** Use exactly these words; downstream tooling
matches on them.
...
`not_found` is **not** `fabricated`. Two failed searches are not proof of
absence. Never write `fabricated` in this bank — record what you searched and
let a later pass overturn it.

`not_a_locatable_work` is the opposite guard, and it is the one that makes this
requirement portable across domains. Some entries name a worked example, a
practice, or a field-wide tension rather than a citable document — there is no
DOI for "the frameworks-as-crutches critique". Demanding an identifier there
forces either an invented one or the loss of the dissenting material, which is
the material an argumentation engine most wants. **The absence is the accurate
record, not a gap.** Expect `WORKED EXAMPLE / CASE` and
`DISSENTING / COMPLICATING` to use this status often in craft domains; expect
`FOUNDATIONAL` and `CONTEMPORARY TREATMENT` to use it almost never.

**Identifier forms, in order of preference:** `doi:` for papers (`10.…`),
`work_id:` for books (`openlibrary:OL…W`) plus `isbn_13:` for the edition
actually used, `url:` for web sources (`https://…`).

**Do not accept the top search hit.** Filter by publication year before
choosing: an unfiltered search returns reprints, translations and preprint
copies that are plausible and wrong. If two records remain defensible, the
status is `ambiguous` and you record **both candidates and choose neither** —
picking asserts something nobody verified. If no edition matches the recorded
year, leave `isbn_13` out and say why in `isbn_note` rather than supplying an
ISBN for a different edition.

**Verify every locator you write.** A guessed URL that happens to return 200 is
indistinguishable from a fact; a guessed one that 404s is the check working.
...
FOUNDATIONAL: 
  [Author(s), Title, Year]
  identifier: [doi: 10.… | work_id: openlibrary:OL…W (+ isbn_13: …) | url: https://…]
  resolution: [status] — checked [YYYY-MM-DD] — via [search method]
...
WORKED EXAMPLE / CASE:
  [Author(s), Title, Year — or the name of the case, campaign or practice]
  identifier: [as above, or omit entirely if not_a_locatable_work]
  resolution: [status] — checked [YYYY-MM-DD] — via [search method]
    note: [required when not_found or not_a_locatable_work — for
           not_a_locatable_work, why there is no document to locate]
  — [Annotation: what this demonstrates]

DISSENTING / COMPLICATING (if found):
  [Author(s), Title, Year — or the name of the debate or tension]
  identifier: [as above, or omit entirely if not_a_locatable_work]
  resolution: [status] — checked [YYYY-MM-DD] — via [search method]
    note: [required when not_found or not_a_locatable_work]
...
ADDITIONAL (if search surfaced strong relevant sources):
  ...
  For `ambiguous`, replace the identifier line with:
  candidates: [list every defensible record, with the fields that would
               settle it — year, publisher, language, edition count]
  resolution: ambiguous — checked [YYYY-MM-DD] — via [search method]
...
IDENTIFIER COVERAGE — report every count against its denominator,
  never as a bare number:
  resolved (identifier fetched and matched): [N] of [total entries]
  ambiguous (candidates recorded, none chosen): [N] of [total entries]
  not_found (searched, not located): [N] of [total entries]
  not_a_locatable_work (no document to locate): [N] of [total entries]
  The four must sum to [total entries]. A zero here means "measured and
  none", which is only readable beside what it was measured over.
```
...
□ Resolution status: does EVERY reference-bank entry carry one
  of resolved / ambiguous / not_found / not_a_locatable_work,
  with the date it was checked and the method used?
  [N] of [total] — list any entry with no status.
□ Identifier presence: every `resolved` entry carries a doi,
  work_id or url? [N] of [N resolved] — an entry marked
  resolved with nothing to resolve is an empty verification
  that reads as a passing one.
□ Reasons recorded: every `not_found` and every
  not_a_locatable_work entry carries a note saying what was
  searched, or why there is no document to locate?
  [N] of [total in those two states]
□ No entry marked `fabricated`? [yes/no]
...
═══════════════════════════════════════════════════════════
SESSION START PROTOCOL (mandatory for every generation 
session after Session 0)
═══════════════════════════════════════════════════════════

**NEW IN v3.26** — Before generating ANY new chapter content in a session:

1. LOAD all chapter fingerprints generated so far
2. LOAD the Voice Calibration Document
3. LOAD the Forward Hook Ledger — current state
4. LOAD Forward Reference Matrix connections relevant 
   to this session's chapters
5. READ the most recent completed chapter's final 2–3 
   paragraphs (the actual prose, not just the fingerprint). 
   This re-activates the generator's sense of the guide's 
   voice.
   
   FALLBACK: If the most recent chapter's prose is not 
   available (context compaction), read the Voice Sample 
   + the most recent fingerprint's tone markers. 
   Less effective but better than nothing.

6. VOICE WARM-UP: Generate a 50–100 word throwaway 
   paragraph in the guide's voice on ANY topic from 
   the upcoming chapter. Do NOT include this in the guide. 
   Its purpose is to re-activate the voice pattern before 
   the reader sees any output. Discard after writing.

Only after completing all 6 steps: begin chapter generation.

═══════════════════════════════════════════════════════════
CHAPTER FINGERPRINT (required — generate immediately after 
completing each chapter)
═══════════════════════════════════════════════════════════

**NEW IN v3.26 — Highest-impact single change.** After completing Chapter [N], produce this structured summary (~200–300 words). Fingerprints for ALL completed chapters MUST remain in context for all subsequent generation. Highest-priority content for context compaction.

```
CHAPTER [N] FINGERPRINT:

1. CORE INSIGHT (action title):
   [The chapter's action title]

2. EMOTIONAL BEAT: [from architecture]
   Achieved? [yes / partially / no — note what prevented 
   full delivery]

3. CONCEPTS DEFINED (first-mention registry):
   [Every concept introduced FIRST TIME, with EXACT 
   definition as written in the chapter text]
   - "TERM: exact definition as it appears"
   - "TERM: exact definition as it appears"

4. CONCEPTS REFERENCED (from earlier chapters):
   [Every concept from a prior chapter that this chapter 
   references, with chapter of origin]
   - "Term (Chapter N)" — context of reference

5. KEY EXAMPLES AND ANALOGIES:
   [Major examples, cases, analogies — enough detail for 
   precise cross-reference in later chapters]

6. VYĀPTIS TREATED:
   - Core: V[N] — [specific formulation as stated]
   - Referenced: V[N] — [how used]

7. HETVĀBHĀSAS TREATED:
   - Introduced: H[N] — [specific formulation]
   - Referenced: H[N] — [context]

8. SPACED RETURNS EXECUTED:
   - SR[N]: Revisited [concept] from Ch [M]. 
     Original: [what Ch M taught]. 
     Elevated: [what this chapter added].

9. TACIT KNOWLEDGE BLOCKS:
   - TK[N]: "[description of tacit insight]"

10. SIMULATIONS (AAR-enhanced):
    - SIM[N]: "[Scenario]"
      Expert sees: [V-numbers, H-numbers]
      Likely errors: [which AAR categories]
      Key divergence point: [specific moment]
      Forward commitment: "[IF-THEN-BEFORE summary]"

11. TONE MARKERS:
    - Formality: [1–5]
    - Analogy density: [low / moderate / high]
    - Dominant analogy domain: [ML, topology, physics...]
    - Emotional register: [matter-of-fact / warm / urgent / 
      provocative / contemplative]
    - Reader address: [direct 2nd person / inclusive "we" / 
      impersonal]

12. FORWARD HOOKS:
    [From Forward Reference Matrix — seeds planted]
    - "Asked [question] → resolves in Ch [M]"
    - "Introduced [concept] simplified → full in Ch [M]"

13. APPENDIX CONTRIBUTIONS:
    - Reference entries: [ratios, formulas, key metrics]
    - Diagnostic questions: [from simulations]
    - Collaborative hooks: [discussion prompts with context]
    - Quick-reference: [concept definitions, framework summaries]
```

**FINGERPRINT USAGE RULES:**

RULE 1 — PRESERVATION: Fingerprints are HIGHEST PRIORITY 
for context preservation. Above architectural documents, 
above the meta-prompt itself.

RULE 2 — FIRST-MENTION CHECK: Before introducing any 
concept, check all prior fingerprints' "CONCEPTS DEFINED." 
Already defined → REFERENCE, don't re-explain. Not defined 
→ define fresh. Needs extension → explicitly note 
"extending Chapter N's treatment."

RULE 3 — TERMINOLOGY LOCK: Use EXACT terms from the 
first-mention registry. "Value Equation" stays "Value 
Equation" — never "value relationship" or "revenue-cost 
identity."

RULE 4 — CROSS-REFERENCE PRECISION: Reference earlier 
chapters' examples by specific details from fingerprints. 
"WeWork's community-adjusted EBITDA" not "a well-known 
growth trap."

RULE 5 — SPACED RETURN VERIFICATION: Before writing a 
planned SR, check the original chapter's fingerprint. 
Must acknowledge, add new dimension, use consistent terms.

RULE 6 — TONAL CONTINUITY: Before each chapter, review 
tone markers from most recent 2–3 fingerprints. Match 
formality, analogy density, register, address style.

RULE 7 — SESSION HANDOFF: Session summaries MUST include 
all fingerprints verbatim + current Forward Hook Ledger + 
Voice Sample reference.

═══════════════════════════════════════════════════════════
FORWARD HOOK LEDGER (maintained across all sessions)
═══════════════════════════════════════════════════════════

**NEW IN v3.26** — Track all cross-chapter forward references:

```
PLANTED HOOKS:
  Hook 1: [description] — planted Ch [N], target Ch [M]
    Status: [OPEN / RESOLVED in Ch [M]]
  Hook 2: [description] — planted Ch [N], target Ch [M]
    Status: [OPEN / RESOLVED in Ch [M]]
  ...

GENERATION RULES:

1. Before starting each chapter: check ledger for OPEN 
   hooks targeted at this chapter. Resolve them.

2. Check Forward Reference Matrix for hooks to 
   PLANT in this chapter. Plant them.

3. After generation: update ledger (new plants, resolved 
   hooks).

4. After fingerprinting: verify ledger matches fingerprint's 
   Forward Hooks section.

STAGE 7 VERIFICATION:
All hooks must be RESOLVED by final chapter. Any OPEN hook:
  (a) Genuine forward reference to reader's future practice 
      → acceptable, mark as "READER-RESOLVED"
  (b) Broken promise → either resolve in final chapter or 
      REMOVE the original hook via revision
```

═══════════════════════════════════════════════════════════
SIMULATION QUALITY CHECKLIST (verify for EVERY simulation)
═══════════════════════════════════════════════════════════

**NEW IN v3.26** — Before finalizing any transfer test or simulation:

□ GENUINE AMBIGUITY: More than one defensible answer? If 
  the "right" answer is obvious to anyone who read the 
  chapter, the simulation is too easy. Revise.

□ NON-OBVIOUS EXPERT INSIGHT: Does the expert analysis 
  reveal something a careful reader would NOT have seen? 
  If it's just "apply the framework correctly" → testing 
  recall, not judgment. Revise.

□ NOVICE-EXPERT DIVERGENCE POINT: Can you identify the 
  SPECIFIC moment where reasoning paths diverge? Not "the 
  novice gets it wrong" but "at THIS observation, the expert 
  notices X while the novice focuses on Y."

□ REAL-WORLD MESSINESS: Includes irrelevant information, 
  ambiguous signals, or conflicting evidence? Real 
  situations don't present clean data. If the simulation 
  gives exactly the needed info and nothing else → textbook 
  problem, not judgment exercise. Revise.

□ READER CONTEXT FIT: Matches the reader's operating 
  context from Phase 1? Indian AI startup founder gets 
  Indian AI startup scenarios, not generic MBA cases.

If ANY check fails → revise the simulation before 
proceeding. A weak simulation is worse than none — it 
creates false confidence.

═══════════════════════════════════════════════════════════
TACIT KNOWLEDGE QUALITY TEST (verify for EVERY TK block)
═══════════════════════════════════════════════════════════

**NEW IN v3.26** — Before finalizing any Tacit Knowledge block:

□ DERIVABILITY CHECK: Could this content be generated by 
  someone who understood the frameworks but had NO real-
  world domain experience?
  
  YES → It's analysis, not tacit knowledge.
    Options:
    (a) Revise to include genuinely tacit elements 
        (perceptual skills, felt senses, timing judgments, 
        emotional signals, patterns resisting articulation)
    (b) Relabel as "Expert Analysis" — honesty about 
        what's genuinely tacit vs. analytically derived

□ FELT EXPERIENCE: Does it describe something the reader 
  would recognize in body or emotions, not just reasoning?
  
  Genuinely tacit:
    "The emotional experience is grief — watching someone 
    do a task worse than you would and not intervening"
    "The tell is genuine laughter vs. performative laughter"
  
  Not tacit (just good analysis):
    "Experienced executives consider multiple stakeholders"
    "The key is aligning incentives with outcomes"

If quality test reveals the TK block is analytical → 
this is FINE. Not every chapter has genuinely tacit 
content. An honest "Expert Analysis" block is better 
than a fake "Tacit Knowledge" block.
...
WHY STOP HERE (required in Structure A):

After presenting the reason, explicitly classify 
what lies ONE LEVEL DEEPER:

"We stop here because the next 'why' leads to [one of]:
  □ An axiom (Type 1 domain) — logically necessary, 
    cannot be otherwise
  □ A fundamental force (Type 2) — physical/mathematical 
    constraint
  □ An empirical brute fact (Type 3) — observed but not 
    derivable from deeper principles
  □ A goal structure (Type 4) — 'because we're trying 
    to achieve X'
  □ A methodological commitment (Type 5) — 'because we've 
    chosen to analyze it this way'"

WHY THIS MATTERS: It tells the reader what KIND of 
foundation they've reached. "We stop because this is 
empirical" means new evidence could change it. "We stop 
because this is axiomatic" means it's definitionally 
true. This distinction is essential for knowing which 
of the guide's principles are revisable and which aren't.

Note: The full 3-level why-chain is OPTIONAL (encouraged 
when illuminating, not required when it forces padding). 
The "WHY STOP HERE" classification is MANDATORY for all 
Structure A chapters.
────────────────────────────────────────────────
...
[Expert reasoning — MUST explicitly name:
  - Which vyāpti(s) applied (V-number and name)
  - Which scope conditions checked
  - Which hetvābhāsa(s) avoided (H-number and name)
  - The specific perceptual/judgment move distinguishing 
    expert from novice]

───────────────────────────────────────────────────────────
IF YOUR REASONING MATCHED — VERIFY YOUR PATH:
───────────────────────────────────────────────────────────

Did you arrive here for the RIGHT reasons?

□ Did you apply [V-number] because you recognized [specific 
  pattern], or reach the same conclusion via different 
  reasoning? Different path = FALSE POSITIVE — will fail 
  in the next novel case.

□ Did you avoid [H-number] because you recognized the trap, 
  or because it didn't occur to you? If the latter, you 
  got lucky — it will catch you when the scenario shifts.

───────────────────────────────────────────────────────────
IF YOUR REASONING DIVERGED — DIAGNOSE THE GAP:
───────────────────────────────────────────────────────────

Step 1 — CLASSIFY THE ERROR:

□ WRONG PATTERN: Applied V[X] when V[Y] was operative.
  → Signal missed: [observable distinguishing VX from VY]
  → Why VX seemed right: [deceptive surface feature]
  → Discrimination skill: [what to check FIRST]
  → CONNECTS TO: Discrimination Case CP[N] if applicable

□ SCOPE VIOLATION: Right vyāpti, outside its scope.
  → Boundary crossed: [specific scope condition violated]
  → Why you missed it: [what made it LOOK in-scope]
  → Habit to develop: Before applying [vyāpti], verify 
    [specific condition]

□ HETVĀBHĀSA: Fell into H[N].
  → Attractive because: [scenario feature making fallacy 
    seem like valid reasoning]
  → Tell missed: [signal distinguishing fallacy from 
    valid reasoning]
  → Catch pattern: When [trigger], pause and ask: 
    "Am I committing H[N]?"

□ MISSING CONCEPT: Needed [concept] from Chapter [N].
  → Why needed: [how it changes reasoning]
  → Connection missed: [cross-domain transfer link]
  → REVISIT: [specific section] → reattempt simulation

Step 2 — TRACE THE DECISION POINT:
At what SPECIFIC moment did your reasoning diverge from 
the expert's? What did you see/weigh (or fail to) at 
that moment? This is your current LEARNING EDGE.

Step 3 — FORWARD COMMITMENT:
"Next time I encounter [surface pattern of this scenario], 
I will first check [specific thing] before applying 
[specific framework element]."

  IF [specific trigger]
  THEN [specific cognitive action]
  BEFORE [specific framework application]

Not "I'll be more careful." A specific IF-THEN-BEFORE rule.

🔍 CHALLENGE THIS PRINCIPLE: Can you think of a situation 
where the regularity applied in this chapter would NOT 
hold — a case fitting the surface pattern but where the 
regularity fails?

If you can: you've found a scope condition. Check it 
against the scope conditions listed above. Already listed? 
Your instinct for limits is calibrating well. NOT listed? 
You may have found a genuine edge case — note it and test 
against the guide's other regularities.

If you can't: try inverting each scope condition. Imagine 
a specific condition is violated. What happens to the 
regularity?
...
For chapters assigned the THRESHOLD emotional beat, the chapter 
structure includes a mandatory RESISTANCE SEQUENCE. Resistance 
is not an add-on — it IS the first 45% of the chapter. The 
concept is presented as resolution of resistance, not as 
information delivered to passive reader.
...
MANDATORY RESISTANCE SEQUENCE:

1. THE READER'S CURRENT BELIEF (~20% of chapter):
   State the pre-threshold understanding CHARITABLY. 
   Not a straw man — a reasonable position with genuine 
   evidence. The reader should think: "Yes, that's 
   exactly what I believe."

2. THE CRACK (~15% of chapter):
   A specific case, observation, or logical consequence 
   where the current belief produces a wrong prediction 
   or contradiction. Must be FELT, not merely intellectual. 
   Use scenario, case study, thought experiment — 
   emotional engagement, not just analytical objection.

3. THE LIMINAL STATE (~10% of chapter):
   Acknowledge the reader holds two incompatible beliefs. 
   Name the discomfort. Validate it: "If you feel uncertain, 
   that's the learning. The temptation is retreating to the 
   familiar. Resist — not because the familiar is wrong, 
   but because it's incomplete."

4. THE NEW UNDERSTANDING (~40% of chapter):
   Teach the threshold concept AS RESOLUTION of the 
   discomfort. Should feel like relief: "Oh — THAT's why 
   the old model broke."

5. INTEGRATION (~15% of chapter):
   Show the new understanding CONTAINS the old as a special 
   case. The pre-threshold belief wasn't wrong — it was a 
   local approximation valid within specific scope conditions. 
   Prevents the reader from feeling their prior understanding 
   was foolish.
...
□ Every chapter has an action title (C1) — passes 3 tests?
□ Every chapter achieved its assigned emotional beat (C3)?
□ Every chapter has a seam bridge to its predecessor
□ Every chapter has a causal reasoning check (3 moves)
□ Every foundational chapter has a transfer test with AAR (C16)
□ All Structure A chapters have WHY STOP HERE (C20)?
...
□ Challenge prompts present after each vyāpti's first application (C18)?
□ Tacit knowledge blocks: [count] present vs. [minimum required]
  ...
  - TK Quality Test passed for each block (C14)? [yes/no]
...
□ Simulation quality checklist passed for all simulations (C13)?
□ Chapter fingerprints generated for ALL completed chapters (C11)?
□ Forward Hook Ledger updated (C12)?
□ Appendix contributions recorded in fingerprints (C21)?
...
**FRAMEWORK INTENT — PART II (reader-facing, at Part transition):**

**NEW IN v3.26** — At the start of each Part, include a Framework Intent Statement:

```
After this Part, you will be able to [specific reasoning 
capability — a cognitive move, not a concept list].

Specifically: when you encounter [type of situation], you 
will [what the reader does differently]. This works because 
Part [N] builds [specific intellectual infrastructure — 
name which vyāptis and skills are now operational].

What this does NOT yet give you: [explicit limit — what 
Parts N+1 through V will add].
```

GENERATION RULE: Each Framework Intent must be traceable 
to specific architectural elements. The "what you can do" 
references vyāptis by name. The "what this doesn't give 
you" references specific later capabilities. Plan at Stage 2 
alongside recalibration checks. Verify at Stage 7. (~100 words each.)
...
**FRAMEWORK INTENT — PART III (reader-facing, at Part transition):**
```
After this Part, you will be able to [specific reasoning 
capability — a cognitive move, not a concept list].
...
Specifically: when you encounter [type of situation], you 
will [what the reader does differently]. This works because 
Part III builds [specific intellectual infrastructure].
...
**Inductive Bias Disclosure (restructured in v3.26):**
```
...
FRAMEWORK: [Name]
ASSUMPTION: [What the framework takes for granted]
OBSERVABLE WHEN VIOLATED: [Specific things you'd see or 
  experience indicating this assumption doesn't hold]
WHAT TO DO: [Concrete alternative — not "be careful" but 
  specific action]
...
□ All chapters have action titles (C1) and achieved emotional beats (C3)?
□ All transfer tests have AAR format (C16)?
□ All THRESHOLD chapters follow 5-part resistance sequence (C15)?
...
□ All planned debates have crux identification (C4)?
□ Challenge prompts present after each vyāpti's first application (C18)?
□ Tacit knowledge blocks: [count] present vs. [minimum required]
  - Each block has case study? [yes/no per block]
  - Each block has experiential simulation (if Medium+ density)? [yes/no]
...
□ Inductive bias disclosure present (restructured format with OBSERVABLE violations)?
□ At least 1 Structure D (Debate) chapter for Types 3, 4, 5
□ Framework Intent Statements present at Part II and Part III transitions (C19)?
□ Simulation quality checklist passed for all simulations (C13)?
□ Chapter fingerprints generated for ALL completed chapters (C11)?
□ Forward Hook Ledger updated (C12)?
□ Appendix contributions recorded in fingerprints (C21)?
...
3. List REMAINING ELEMENTS for Stage 6, specifically:
   ...
   - Scenario forks for all ? and ⚡ entries (C17)
   - Framework Intent Statements for remaining Parts (C19)
   ...
   - Final appendix assembly from fingerprint contributions (C21)
...
───────────────────────────────────────────────────────────
SCENARIO FORKS (required for every ? and ⚡ entry above)
───────────────────────────────────────────────────────────

NEW IN v3.26 — For each genuinely open or actively contested 
question that AFFECTS the framework, provide a scenario fork:

OPEN QUESTION: [statement]
CURRENT STATUS: [? genuinely open / ⚡ actively contested]
WHY IT MATTERS: [which part of the framework depends on 
  this resolution — if none, this doesn't need a fork]

SCENARIO A: [Resolution A — specific claim]
  IF this resolves as [A]:
    FRAMEWORK IMPACT:
    → V[N] holds as taught / needs modification: [specific]
    → Chapter [M]'s treatment becomes [more/less complete]
    PRACTICAL IMPLICATIONS:
    → Strategy should [specific implication]
    MONITOR FOR:
    → [Observable signal 1 — readable in news, data, market]
    → [Observable signal 2]
    SIGNAL SOURCES: [where to watch]

SCENARIO B: [Resolution B — specific claim]
  [Same structure as A]

[SCENARIO C — optional, only if genuinely different 
framework impact exists]

CURRENT EVIDENCE LEANS: [A / B / genuinely balanced — 
be honest, not artificially balanced]
WHAT WOULD SETTLE IT: [specific evidence or event]
REVIEW CADENCE: [quarterly / annually / trigger-based]

GENERATION RULES:
1. Only fork questions that AFFECT framework (not just 
   interesting open questions)
2. Monitoring signals must be OBSERVABLE
3. Honest evidence lean (no false balance)
4. Max 2–3 scenarios per fork
5. Connect to operating cadence for periodic review
6. ~200–350 words per fork
```
...
**NEW IN v3.26** — This appendix collects all discussion and teaching exercises for the guide. Each entry is tied to a specific chapter and is designed for use with peers, study groups, or teaching contexts. The guide is fully functional for solo readers without this appendix.

### INCREMENTAL APPENDIX ACCUMULATION

**NEW IN v3.26** — As each chapter is completed, immediately generate that chapter's APPENDIX CONTRIBUTIONS as part of the fingerprint (Section 13):

- Reference entries: ratios, formulas, metrics introduced
- Diagnostic questions: from simulations and transfer tests
- Collaborative hooks: discussion prompts with chapter context
- Quick-reference: concept definitions, framework summaries

These are stored in the fingerprint's Section 13.

FINAL APPENDIX ASSEMBLY (end of guide):
COMPILE contributions from all fingerprints. This is a 
compilation task, not a generation task. Each contribution 
was generated when chapter context was fresh — the assembly 
just organizes them.

APPENDIX ORGANIZATION: Reference tables organized BY 
VYĀPTI (which items inform which regularities), not by 
conventional categories. Diagnostic questions cross-
referenced to specific simulations. Collaborative hooks 
include chapter and case study context.
...
2. All ? and ⚡ entries have scenario forks (C17)?
3. Resolution ledger: ALL entries accounted for (resolved or 
   open)? Display the FINAL STATUS table.
4. Part V subsection check:
...
5. Framework Intent Statements present for all Parts (C19)?
6. Collaborative Learning Appendix check:
...
7. Final appendix assembly compiled from fingerprint 
   contributions (C21)? Organized by vyāpti? [yes/no]
8. Chapter fingerprints complete for ALL chapters (C11)?
9. Forward Hook Ledger: all hooks RESOLVED or READER-RESOLVED (C12)?
10. Produce FINAL ELEMENT TRACKING — all counts should equal 
   totals. List any gaps.

STOP and present using Rule 1 format.

Tell the user: "All stages complete. Reply 'continue' for 
Stage 7 (Quality Verification), or specify corrections."
...
**v3.26 PREVENTION VERIFICATION**

FINGERPRINT INTEGRITY:
□ All chapters have fingerprints?
□ Tone markers consistent across chapters (no drift >1 
  point on formality scale between adjacent chapters)?
□ First-mention registry has no duplicate definitions 
  (same concept defined from scratch in multiple chapters)?

FORWARD HOOKS:
□ All hooks in ledger RESOLVED or marked READER-RESOLVED?
□ No broken promises (hooks planted but never harvested)?

AAR QUALITY:
□ AAR present for every transfer test?
□ V-numbers and H-numbers named in every AAR?
□ At least one error category with scenario-specific detail?
□ All forward commitments use IF-THEN-BEFORE?
□ AARs reference CP-numbers where confusion-pairs are 
  the likely error source?

EMOTIONAL ARC:
□ Intended beats achieved? (check fingerprint beat 
  achievement field)
□ Any chapters where beat wasn't delivered → flag for 
  Stage 8 if needed

SIMULATION QUALITY:
□ All simulations passed the 5-point quality checklist 
  during generation?

THRESHOLD CONCEPTS:
□ All THRESHOLD chapters follow the 5-part resistance 
  sequence?
□ Resistance ≥ 30% of chapter space?

DEBATES:
□ All debates have identified cruxes?
□ No scope-based "debates" disguised as genuine disagreement?

SCENARIO FORKS:
□ All ? and ⚡ entries in Part IV have forks?
□ All monitoring signals are observable?

FRAMEWORK INTENTS:
□ Every Part has a Framework Intent Statement?
□ Each traces to specific vyāptis?

STRESS-TESTING:
□ Challenge prompts present after each vyāpti's first 
  application?
□ Part III bias disclosure uses restructured format?
...
# ═══════════════════════════════════════════
# STAGE 8: TARGETED REVISION (Safety Net)
# ═══════════════════════════════════════════

**NEW IN v3.26** — If all prevention mechanisms (C1–C21) work correctly, Stage 8's revision manifest should be EMPTY. Stage 8 exists as verification that the prevention stack worked, and as a safety net for the rare cases where it didn't.

### STEP 8A — GENERATE REVISION MANIFEST

Review all fingerprints. Check six revision types:

```
TYPE 1 — CALIBRATION (early vs. late chapter quality):
  Compare fingerprint tone markers across chapters.
  If early chapters show lower analogy density, different 
  formality, or simpler simulations → flag.
  PREVENTION: Voice Calibration + Session Start Protocol 
  should have prevented this.

TYPE 2 — CROSS-CHAPTER CONNECTIONS:
  Check Forward Hook Ledger for any OPEN hooks.
  Check Forward Reference Matrix for unexecuted connections.
  PREVENTION: Forward Reference Matrix + Hook Ledger 
  should have prevented.

TYPE 3 — REDUNDANCY:
  Check first-mention registries for duplicate definitions.
  PREVENTION: Fingerprint Rule 2 should have prevented.

TYPE 4 — APPENDIX QUALITY:
  Check appendix contributions for completeness and 
  cross-referencing.
  PREVENTION: Incremental Accumulation should have 
  prevented.

TYPE 5 — THRESHOLD RESISTANCE:
  Check THRESHOLD chapters for ≥30% resistance.
  PREVENTION: Threshold Resistance Structure should have 
  prevented.

TYPE 6 — DEBATE CRUX:
  Check debates for genuine cruxes.
  PREVENTION: Debate Crux Identification should have 
  prevented.
```

### STEP 8B — IF MANIFEST IS NOT EMPTY

Execute revisions surgically in order:
  Type 1 → Type 5 → Type 2 → Type 3 → Type 6 → Type 4

After each revision, update affected fingerprint.

### STEP 8C — DIAGNOSTIC

For each revision item, diagnose WHY the prevention failed:
  - Was the prevention mechanism not followed? 
    → Process problem, reinforce the protocol
  - Was the prevention mechanism insufficient? 
    → Architecture problem, note for v3.27
  - Was this genuinely unpreventable? 
    → Accept as irreducible cost of sequential generation

═══════════════════════════════════════════
⛔ STAGE 8 GATE — HARD STOP
═══════════════════════════════════════════

□ Manifest generated
□ All items resolved (or documented as KNOWN LIMITATIONS)
□ Fingerprints updated
□ Diagnostic completed for any non-empty manifest items

STOP and present the revision manifest (even if empty) and 
diagnostic results.
...
**NEW IN v3.26** — Use this prompt to expand any proof sketch from the Derivation Registry into a full 6-component Ānvīkṣikī derivation. Paste it into a new conversation along with: (1) the generated guide document, (2) the Stage 2 architecture output, and (3) the claim number you want expanded.
...
## APPENDIX G: v3.26 CHANGELOG

Changes from v3.26, with rationale:
...
## APPENDIX H: v3.26 CHANGELOG

Changes from v3.26, transforming the pipeline from forward-only generation with post-hoc revision into a PREVENTION-FIRST pipeline that produces final-quality output in a single forward pass:

| Change | Rationale |
|---|---|
| **C1 Action Titles** (Step 2G) | Chapter titles as complete insight-stating clauses. 3 tests: Disagreement, Domain Specificity, Compression. Source: Minto Pyramid. |
| **C2 MECE Assertion** (Step 2C) | Exhaustiveness mandatory, exclusivity advisory at dependency map branch points. Source: McKinsey MECE. |
| **C3 Emotional Arc** (Step 2G) | 5-beat vocabulary (Challenge/Reward/Rest/Threshold/Expansion) with 4 sequencing constraints. Source: IDEO journey mapping. |
| **C4 Debate Crux ID** (Step 2G) | Crux statement + type classification required for all debates. Scope-based → contextual analysis. Source: intelligence analysis. |
| **C5 Forward Reference Matrix** (Step 2N) | All cross-chapter connections planned pre-generation with 5 connection types. Enables precise forward/back references. |
| **C6 Exit Narrative** (Step 2O) | 150–200 word simulation of completed reader with V/H-number references and 7-item verification. Source: Amazon Working Backwards. |
| **C7 Session Plan** (Step 2P) | Chapter-to-session assignment with fatigue, difficulty, and transition constraints. |
| **C8 Epistemic Status** (vyāpti defs) | Confidence levels + evidence quality + guide rendering rules for calibrated language. |
| **C9 Voice Calibration** (pre-Stage 3) | 500–800 word voice sample establishing tone before Chapter 1. Prevents cold-start quality degradation. |
| **C10 Session Start Protocol** (Stages 4–6) | 6-step mandatory protocol: load fingerprints, voice sample, hook ledger, matrix connections, recent prose, voice warm-up. |
| **C11 Chapter Fingerprints** (Stages 4–6) | 13-section structured summary per chapter. Highest-impact change (8–12%). Solves context window pressure across all dimensions. |
| **C12 Forward Hook Ledger** (Stages 4–6) | Running ledger tracking planted/resolved cross-chapter promises. Prevents orphaned forward references. |
| **C13 Simulation Quality Checklist** (Stages 4–6) | 5-point verification: ambiguity, non-obvious insight, divergence point, messiness, context fit. |
| **C14 Tacit Knowledge Quality Test** (Stages 4–6) | Derivability check + felt-experience test. Honest "Expert Analysis" label when content isn't genuinely tacit. |
| **C15 Threshold Resistance Structure** (Stages 4–6) | 5-part mandatory sequence for THRESHOLD chapters: Current Belief → Crack → Liminal State → New Understanding → Integration. Resistance = first 45% of chapter. |
| **C16 AAR Reflection** (all structures) | 4-category error taxonomy + divergence tracing + IF-THEN-BEFORE forward commitment. Source: U.S. Military AAR + Gollwitzer implementation intentions. |
| **C17 Scenario Forks** (Part IV) | 2–3 resolution scenarios per open question with framework impact, monitoring signals, evidence lean, review cadence. Source: BCG scenario planning. |
| **C18 Stress-Testing Prompts** (after vyāpti application) | Reader-initiated principle challenging + restructured bias disclosure with observable violations. Source: Bridgewater radical transparency. |
| **C19 Framework Intent** (Part transitions) | 5 reader-facing statements: what you can now do, why, what's still missing. Source: U.S. Military Commander's Intent. |
| **C20 Why-Chain WHY STOP HERE** (Structure A) | Mandatory domain-type classification of the foundation beneath each Hetu. Full 3-level chain optional. Source: Toyota 5 Whys. |
| **C21 Incremental Appendix** (Stages 4–6 + Fingerprints) | Per-chapter appendix contributions in fingerprints. Final assembly = compilation, not generation. |
| **Stage 8 Safety Net** | 6-type revision manifest as verification of prevention stack. Should produce empty manifest if C1–C21 work correctly. Includes diagnostic for prevention failures. |

---

## APPENDIX I: v3.27 CHANGELOG

Changes from v3.26, with rationale:

| Change | Rationale |
|---|---|
| **Reference-bank entries require a machine-checkable identifier** (Step 3E) | v3.26 asked for "URL if available", which made the locator optional and produced banks of title/author/year strings that nothing downstream can confirm. A looked-up citation and an invented one are byte-identical once written, so the bank was well-formed and entirely unverifiable. Entries now carry a DOI, an OpenLibrary work id (plus the ISBN of the edition used), or a URL. |
| **Four resolution statuses, with `not_a_locatable_work` among them** (Step 3E) | Requiring an identifier of every entry does not transfer across domains. Craft banks cite worked examples, practices and field-wide tensions — there is no DOI for a critique with no single source — and demanding one forces either a fabricated identifier or the loss of the dissenting material, which is the material an argumentation engine most needs. The fourth status records the absence as accurate rather than as a gap. `not_found` is kept deliberately distinct from `fabricated`: two failed searches are not proof of absence. |
| **Year-filtering and candidate recording required before choosing** (Step 3E) | An unfiltered search returns reprints, translations and preprint copies that are plausible and wrong; accepting rank 1 writes a wrong identifier inside the mechanism meant to detect wrong identifiers. Where several records stay defensible the status is `ambiguous`, both candidates are recorded, and neither is chosen. |
| **Identifier coverage reported against its denominator** (Sourcing Quality Summary) | A bare count of resolved sources cannot be told from a count that was never taken. The four statuses must sum to the entry total, so a zero reads as "measured and none" rather than as silence. |
| **Stage 3 gate audits resolution, identifiers and reasons** (Stage 3 Gate) | A requirement stated in the template and unchecked at the gate is a requirement the generator can skip without anything noticing — the gate is the only place in the stage that forces a count. |
