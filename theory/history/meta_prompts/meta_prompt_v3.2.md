╔══════════════════════════════════════════════════════════╗
║  ĀNVĪKṢIKĪ v3.2 — ENFORCED STAGE GATE PROTOCOL          ║
╠══════════════════════════════════════════════════════════╣
║                                                          ║
...
subsequent stages:
   □ Vyāptis referenced: [N] of [total]
   □ Hetvābhāsas introduced: [N] of [total]
   □ Spaced returns executed: [N] of [total]
   □ Discrimination cases: [N] of [total]
   □ Landscape acknowledgments: [N] of [total]
   □ Decay markers placed: [N] of [total]
   □ Tacit knowledge blocks: [N] of [minimum]
   □ Case studies in tacit blocks: [N] of [N blocks]
   □ Experiential simulations: [N] of [N blocks where required]
   □ High-contrast debates: [N] of [N blocks where required]
   □ Podcast/interview references: [N] of [N blocks where required]
   □ Resolution ledger entries resolved: [N] of [total]
   □ Chapter references: [N] chapters with references of [total] chapters
   □ Reference Bank coverage: [N] chapters drawing from verified sources vs. [N] relying on training knowledge
   □ Derivation notes: [N] of [total from registry]
   □ Cross-domain isomorphism blocks: [N] of [total from map]
   □ Metacognitive checkpoints: [N] of [total chapters]
   □ Collaborative hooks: [N] of [total chapters]
   □ Framework anti-patterns: [N] of [minimum 3]
   □ Composability anchors: sequel [N] + parallel [N]
...
# The Ānvīkṣikī Meta-Prompt for Instructional Design — v3.2
...
The derivation uses six components that the guide itself teaches:
...
7. Threshold concept identification and treatment (at least 1)
8. Part III: Failure Mode Catalogue (at least 3 modes using Structure C)
9. Part IV: Frontier section with epistemic status markers
10. Transfer tests for all foundational chapters

**TIER 3 — Required for full structural compliance:**
11. All landscape acknowledgments
12. All discrimination cases
13. Full spaced return plan executed
14. All decay markers on condition-dependent claims
15. Resolution ledger fully resolved
16. Part II: Full synthesis chapters
17. Error Museum
18. Compression exercise
19. All tacit knowledge markers (minimum 1 per Part for Craft domains)
...
Collaborative hooks are lightweight: one discussion or teaching prompt per chapter, not a full collaborative learning design. The guide remains usable solo; the hooks are available for readers who learn with others.
...
```
╔══════════════════════════════════════════════════════════════════════════╗
║        THE ĀNVĪKṢIKĪ INSTRUCTIONAL DESIGN META-PROMPT v3.2             ║
...
**NEW IN v3.2** — Identify claims in the planned guide that are non-obvious: they rest on multi-step inference, combine multiple vyāptis, or lack direct source grounding. These claims require explicit derivation chains in the guide text.
...
For each, pre-write the derivation skeleton:
...
Deriving from:
  Premise 1: [Statement] 
    — Pramāṇa type: [pratyakṣa / anumāna / śabda / upamāna]
    — Source: [Reference Bank entry / training knowledge]
  Premise 2: [Statement]
    — Pramāṇa type: [type]
    — Source: [source]
Vyāpti chain: [V₁] → [V₂] → conclusion
  — Each step maps to named vyāpti: [which ones]
Causal status of linkage: [Causal / Structural / Empirical regularity]
Graph parents: [Concepts from dependency DAG this depends on]
Hetvābhāsa risk: [Which named fallacy could produce this 
  conclusion invalidly — and why the derivation avoids it]
Verdict: [Why this derivation holds despite the hetvābhāsa risk]
```
...
□ Derivation registry: [count] entries — required: min 3 
  (1 per Part). Each has full derivation skeleton? [yes/no]
...
Derivation Registry, Step 2J — insert at point of claim):
  ...
  Deriving from:
    ...
    Premise 2: [Statement]
      — Pramāṇa: [type]
      — Source: [source]
  Vyāpti chain: [V₁: name] → [V₂: name] → conclusion
  Causal status: [Causal / Structural / Empirical regularity]
  Graph parents: [Concepts from dependency DAG]
  Hetvābhāsa risk: [Named fallacy that could produce this 
    invalidly] — avoided because: [why the derivation 
    escapes this error]
  Source grounding: [Sourced / Partially sourced / Unsourced 
    — if unsourced, reader should verify via [specific method]]
...
COLLABORATIVE HOOK (choose one per chapter):

  🗣️ DISCUSSION PROMPT: [A question for peer debate — structured 
    so both sides are defensible, exercising Vāda mode. 
    Not "what do you think?" but a genuine crux.]
  OR
  🎓 TEACHING PROMPT: "Explain [this chapter's core concept] 
    to someone who hasn't read this guide. Where do you 
    stumble? That's where your understanding is still 
    receptive, not generative."
...
METACOGNITIVE CHECKPOINT (as in Structure A)
COLLABORATIVE HOOK (as in Structure A)
...
□ Derivation notes from registry: [count placed] vs. [count due in Parts II–III]
□ Cross-domain isomorphism blocks: [count placed] vs. [count due]
□ Metacognitive checkpoints: [count] / [total Parts II–III chapters]
□ Collaborative hooks: [count] / [total Parts II–III chapters]
...
4. Produce FINAL ELEMENT TRACKING — all counts should equal 
...
□ Derivation notes: All [N] from registry placed? Each uses full Ānvīkṣikī derivation format (pramāṇa + vyāpti chain + causal status + graph position + hetvābhāsa check + source grounding)? [List any incomplete]
...
□ Metacognitive checkpoints: Present in every chapter? [Count] / [total chapters]. Each has calibration probe + self-explanation prompt? [yes/no]
□ Collaborative hooks: Present in every chapter? [Count] / [total chapters]. Each is a genuine crux (discussion) or teaching prompt? [yes/no]
...
□ Can a reader verify the guide's own non-obvious claims using the derivation notes and the Ānvīkṣikī tools the guide taught them? (Meta-circular verification)
...
These research connections are not exhaustive validation — they are the primary evidence base. The meta-prompt's specific implementations (e.g., the 6-component derivation format, the vyāpti-based isomorphism mapping) extend beyond what any single study validates. Their design is informed by, but not limited to, the research cited.
